...
let myCastedTableView = myView as? UITableView
/*:
 - Experiment:
...
for item in view.subviews {
  if item is UIButton {
    buttonArray.append(item as! UIButton)
  }
}
...
for MediaItem in library {
  print(MediaItem, ":", MediaItem.name, ":", (MediaItem as? Movie)?.director ?? "n/a", ":", (MediaItem as? Song)?.artist ?? "n/a")
}
